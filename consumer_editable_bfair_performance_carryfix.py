...
Copy of consumer_editable_bfair_performance.py with one additional fix:
extra_cols are now reattached inside run_variant itself (via
attach_extra_columns), not just at the reordered_records save site. The
bfairreorder buffer returned by run_variant is what seeds message_buffer
for the *next* window (main loop's `message_buffer, fair_after =
edit_window(...)`), so without this, every window after the first loses
extra_cols on its carried-over window_size prefix (only the freshly-polled
landmark items still had them) -- verified on movie_vote_summary/genre-1
output as ~66% NaN in avg_rating/vote_count/movieId beyond edit_event 0.
Reattaching inside run_variant means the carried-over buffer itself is
complete, so the gap doesn't reappear on the next hop.
...
class Accumulator:
    ...
    def summarize(self, prefix, its, total_blocks, landmark):
        ...
        return {
            ...
            # which was blind to the reorder cost entirely.
...
def run_variant(name, reorder_fn, seq, attr, fairness, floor, ceiling, position, window_size,
                 block_size, landmark, acc: Accumulator, extra_rows=None):
    ...
    edited stream as a list of {attr: value, **extra_cols} records --
    extra_rows[i] (if given) is reattached via attach_extra_columns so the
    returned records stay complete after reordering, since this return
    value is what seeds the next window's message_buffer.
    """
    tracemalloc.start()
    tracemalloc.reset_peak()
    t0 = time.perf_counter()
    ...
    if extra_rows is not None:
        edited_extras = attach_extra_columns(seq, extra_rows, edited)
        edited_df = pd.DataFrame([{attr: v, **e} for v, e in zip(edited, edited_extras)])
    else:
...
if __name__ == "__main__":
    ...
    # Per-topic (not the shared hardcoded group the original script uses) so
    # concurrent/overlapping runs never join the same consumer group and
    # trigger a rebalance on each other.
    GROUP_ID = f"{IN_TOPIC}-group"
    ...
    def edit_window(batch_landmarked, attr):
        ...
        run_variant("before_reorder", identity_reorder, seq, attr, fairness, floor, ceiling, position,
                    WINDOW_SIZE, BLOCK_SIZE, LANDMARK, before_acc, extra_rows=extra_rows)
        ...
        for name, reorder_fn in STRATEGIES.items():
            buffer, local_sketch = run_variant(
                ...
                WINDOW_SIZE, BLOCK_SIZE, LANDMARK, accumulators[name], extra_rows=extra_rows,
            )
        ...
        if save_reordered_path:
            # buffer's records already carry reattached extra_cols (run_variant
            # did it), so no separate reattachment is needed here.
