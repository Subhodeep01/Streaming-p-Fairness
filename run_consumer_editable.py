...
if window_size <= block_size or window_size%block_size!=0 or landmark > window_size or (landmark+window_size)%block_size!=0:
    ...
    command = ['python', 'consumer_editable_bfair_performance.py', '--window_size',f'{window_size}', '--block_size', f'{block_size}', '--topic_name',f'{topic_name}', '--max_windows',f'{max_windows}', '--landmark',f'{landmark}', '--brt_force',f'{brt_force}', '--backward',f'{backward}']
